/*
  ...
  scjurgen@yahoo.com: 
   added ctrl register function (set_ctrlreg) to enable clock out with 1Hz, 4Khz, 8Khz or 32Khz (useful with pin 2 or 3 on arduino for precise clock interrupt )
*/
...
void DS1307::start(void)
{
    ...
    // TODO : preserve existing seconds, now just preserving if previously read
    rtc_bcd[DS1307_SEC]&=~DS1307_CLOCKHALT;
    save_rtc();
}
...
byte DS1307::getSingleReg(byte reg)
{
  // use the Wire lib to connect to tho rtc
  // reset the register pointer to zero
  Wire.beginTransmission(DS1307_CTRL_ID);
...
void DS1307::setSingleReg(byte reg, byte b)
{
	Wire.beginTransmission(DS1307_CTRL_ID);
    Wire.write(reg);
    Wire.write(b);
...
void DS1307::set_ctrlreg(byte b)
{
	byte secs=getSingleReg(DS1307_HI_SEC);
	setSingleReg(DS1307_HI_SEC,secs|DS1307_CLOCKHALT);
	setSingleReg(DS1307_CTRLREG,b);	
	setSingleReg(DS1307_HI_SEC,secs);
}
