...
// переменные, необходимые для работы ds1307
//  DS1307RTC Clock;
bool Century = false;
bool h12;
bool PM;
...
void ReadDS1307()   //функция считывает в глобальные переменные minute и hour текущие значения
{
  //  tmElements_t t=Clock.read(t);
...
void loop() {
  ...
  int keyval = digitalRead(keyb); // считываем значение с клавиатуры
  ...
  if (currentTime >= (startTime2 + 200)) // если прошло 200мс посмотрим какая кнопка была нажата
  {
    ...
    if (keynum == 3) // кнопка переключения между режимами "время" и "настройка"
    {
      ...
      else if (mode == 1) mode = 0; //если был режим настроек, переключаемся на текущее время
    }
  ...
  if (!timeout) //если разрешено, считываем нажатую кнопку
  {
    ...
    */
    if (keyval > 0 && keyval < 400) keynum = 1;
...
void digitalClockDisplay() {
  // digital clock display of the time    second, minute, hour, date, month, year
  Serial.print(hour);
  printDigits(minute);
  printDigits(second);
  Serial.print(" ");
